		...
		var Plugin = {
					...
					enable: function() {
						var enableLock = function(tablePart) {
							...
							$(tablePart).find('.' + pfx + 'datatable-row').each(function() {
								...
								var rowLeft = $('<tr/>').addClass(pfx + 'datatable-row').data('obj', $(this).data('obj')).appen(lockLeft);
								var rowScroll = $('<tr/>').addClass(pfx + 'datatable-row').data('obj', $(this).data('obj')).appen(lockScroll);
								var rowRight = $('<tr/>').addClass(pfx + 'datatable-row').data('obj', $(this).data('obj')).appen(lockRight);
							...
							if (lock.lockEnabled.left.length > 0) {
								...
								$(lockLeft).appen(tablePart);
							}
							if (lock.lockEnabled.left.length > 0 || lock.lockEnabled.right.length > 0) {
								$(lockScroll).appen(tablePart);
							}
							if (lock.lockEnabled.right.length > 0) {
								$(datatable.wrap).addClass(pfx + 'datatable-lock');
								$(lockRight).appen(tablePart);
							}
						};
			...
			setupBaseDOM: function() {
				...
				if ($(datatable).prop('tagName') === 'TABLE') {
					...
					datatable.table = $('<table/>').addClass(pfx + 'datatable-table').appen(datatable);
				}
				...
				if ($(datatable.tableHead).length === 0) {
					datatable.tableHead = $('<thead/>').prepen(datatable.table);
				}

				// create table head element
				datatable.tableBody = $(datatable.table).find('tbody');
				if ($(datatable.tableBody).length === 0) {
					datatable.tableBody = $('<tbody/>').appen(datatable.table);
				}
				...
				if (typeof options.layout.footer !== 'undefined' &&
					...
					if ($(datatable.tableFoot).length === 0) {
						datatable.tableFoot = $('<tfoot/>').appen(datatable.table);
					}
				}
			},
				...
				var pg = {
					...
					populate: function() {
						...
						$.each(Plugin.getOption('toolbar.layout'), function(i, layout) {
							$(pg.pagerLayout[layout]).appen(pg.pager);
						});
						...
						$.each(pageSizes, function(i, size) {
							...
							$('<option/>').attr('value', size).html(display).appen(pageSizeSelect);
						});
			...
			autoHide: function() {
				...
				var toggleHiddenColumns = function(e) {
					...
					if (!$(detailRow).hasClass(pfx + 'datatable-row-detail')) {
						...
						var detailRowTd = $('<td/>').addClass(pfx + 'datatable-detail').attr('colspan', Plugin.getTotalColumns()).appen(detailRow);
